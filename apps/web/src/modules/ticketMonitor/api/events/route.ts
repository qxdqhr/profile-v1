...
import { getCachedTicketEvents } from '@/modules/ticketMonitor/server/getTicketEvents';
...
function parseSources(searchParams: URLSearchParams): TicketSource[] | undefined {
  ...
  const sources = merged.filter((item): item is TicketSource => validSources.includes(item as TicketSource));
