...
import {
  ThemeProvider,
  type Sa2ThemeId,
} from '@sa2kit-ui/theme-runtime';
import { useCallback, useEffect, useState, type ReactNode } from 'react';

const STORAGE_KEY = 'profile-home-v2-theme';

export const HOME_V2_THEME_IDS = ['animal-island', 'jieyuan-garden'] as const;
...
function readStoredTheme(): HomeV2ThemeId {
  ...
  if (saved === 'jieyuan-garden' || saved === 'animal-island') {
    return saved;
  }
...
export function HomeV2ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const handleThemeChange = useCallback((next: Sa2ThemeId) => {
    if (next !== 'animal-island' && next !== 'jieyuan-garden') return;
    setTheme(next);
