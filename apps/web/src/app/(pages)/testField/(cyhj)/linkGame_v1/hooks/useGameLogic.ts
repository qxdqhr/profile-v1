...
import { GRID_WIDTH, GRID_HEIGHT, TILE_SIZE, TILE_GAP, TYPES_COUNT } from '../constant/const'
export const useGameLogic = () => {
    ...
    const canConnect = (tile1: Tile, tile2: Tile, tiles: Tile[]): { canConnect: boolean, path: { x: number, y: number }[] } => {
    ...
    // 检查是否存在可配对的方块
    const hasMatchablePairs = (tiles: Tile[]): boolean => {
