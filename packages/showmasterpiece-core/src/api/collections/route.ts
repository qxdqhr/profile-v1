...
import { collectionsDbService } from '@profile/showmasterpiece-core/masterpiecesDbService';
...
/**
 * 获取画集列表
 * GET /api/showmasterpiece/collections
 * 
 * 查询参数:
...
export async function GET(request: NextRequest) {
  try {
    ...
    const collections = await collectionsDbService.getAllCollections(!nocache);
...
/**
 * 创建新画集
 * POST /api/showmasterpiece/collections
 * 
 * 权限要求: 需要用户登录认证
 * 
 * 请求体限制:
...
export async function POST(request: NextRequest) {
  try {
    ...
    const collectionData = requestBody;
...
/**
 * 批量更新画集操作
 * PATCH /api/showmasterpiece/collections
 * 
 * 权限要求: 需要用户登录认证
 * 
