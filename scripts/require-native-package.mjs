 ...
 * 用法：node scripts/require-native-package.mjs @profile/calendar-mobile
 */
...
if (!pkgName) {
  console.error('usage: node scripts/require-native-package.mjs <package-name>');
  process.exit(1);
}
...
const nativeEnabled =
  /^ {2}- 'app_mobile\/\*'$/m.test(workspace) &&
  /^ {2}- 'app_desktop\/\*'$/m.test(workspace);
