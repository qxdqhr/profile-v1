...
async function createProdTestData() {
  try {
    console.log('开始为生产环境创建测试数据...');

    // 检查是否已有配置数据
    const existingConfig = await db.select().from(comicUniverseConfigs).limit(1);
    if (existingConfig.length > 0) {
      console.log('生产环境已有配置数据，跳过创建');
    } else {
      // 1. 创建配置
      console.log('创建配置...');
      ...
      console.log('生产环境配置创建成功:', config[0]);
    }

    // 检查是否已有分类数据
    const existingCategories = await db.select().from(comicUniverseCategories);
    if (existingCategories.length > 0) {
      console.log('生产环境已有分类数据，跳过创建');
    } else {
      // 2. 创建分类
      console.log('创建分类...');
      ...
      console.log('生产环境分类创建成功:', categories);
    }

    // 检查是否已有画集数据
    const existingCollections = await db.select().from(comicUniverseCollections);
    if (existingCollections.length > 0) {
      console.log('生产环境已有画集数据，跳过创建');
    } else {
      // 获取分类ID
      const categories = await db.select().from(comicUniverseCategories);
      const classicalCategory = categories.find(c => c.name === '古典艺术');
      const modernCategory = categories.find(c => c.name === '现代艺术');
      ...
      const collections = await db.insert(comicUniverseCollections).values([
        {
          ...
          categoryId: modernCategory?.id || null,
          isPublished: true,
        ...
        },
        {
          ...
          categoryId: classicalCategory?.id || null,
          isPublished: true,
          publishedAt: new Date(),
          displayOrder: 3,
          viewCount: 0
      ...
      console.log('生产环境画集创建成功:', collections);
    ...
    console.log('✅ 生产环境测试数据创建完成！');
    process.exit(0);
  } catch (error) {
    console.error('❌ 创建生产环境测试数据失败：', error);
    process.exit(1);
  }
}
