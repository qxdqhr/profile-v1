...
启明星二期（Phase H）：蓝图 [§15](./code-review/libraries/BLUEPRINT-multiplatform-sa2kit.md#15-phase-h--启明星二期主站变薄--接单契约--包体)（**H1 ✅ · H2 ✅ · H∞ ✅（除 mikutap E）**）；主站拆分清单 [`code-review/libraries/WEB-MODULE-INVENTORY.md`](./code-review/libraries/WEB-MODULE-INVENTORY.md)；空目录演练 [`packages/sa2kit/docs/HOST-ONBOARDING-DRILL-H1-P1.md`](../packages/sa2kit/docs/HOST-ONBOARDING-DRILL-H1-P1.md)。OPT 队列已空，见 [`code-review/PENDING-OPTIMIZATION.md`](./code-review/PENDING-OPTIMIZATION.md)。
...
| Utilities | `@profile/utilities` | 3011 | `/tools` |
...
领域逻辑优先在 `sa2kit/business/*`；基建在 `host/{auth,db,config,ui}`。`packages/` npm 库 **仅** `sa2kit` + `sa2kit-ui`（G8 门禁）；另有 `sa2kit-skill`（Agent skills，非 workspace）。业务 *-core **已删**。
