...
| 待定优化（OPT 队列已空；说「优化项目」时若有新项再做） | [`code-review/PENDING-OPTIMIZATION.md`](./code-review/PENDING-OPTIMIZATION.md) |
...
| 现网子应用启停开关 | [`../deploy/runtime-modules.md`](../deploy/runtime-modules.md) |
...
│   ├── mikuGacha/            # 初音抽卡（索引 → sa2kit business）
│   ├── mmd/
│   ├── showmasterpiece/
│   ├── skill-manager/
│   └── ticket-monitor/
