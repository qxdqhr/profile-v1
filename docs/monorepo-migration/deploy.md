...
|------|------------|-------------------|----------|
| web（主站） | `app_web/web/Dockerfile` | `qhr-profile-web:NNN` | 3000 |
| calendar | `app_web/calendar/Dockerfile` | `qhr-profile-calendar:NNN` | 3001 |
| teach-hub | `app_web/teach-hub/Dockerfile` | `qhr-profile-teach-hub:NNN` | 3002 |
| showmasterpiece | `app_web/showmasterpiece/Dockerfile` | `qhr-profile-showmasterpiece:NNN` | 3003 |
...
```bash
docker build -f app_web/web/Dockerfile -t qhr-profile-web:local .
docker build -f app_web/calendar/Dockerfile -t qhr-profile-calendar:local .
docker build -f app_web/teach-hub/Dockerfile -t qhr-profile-teach-hub:local .
docker build -f app_web/showmasterpiece/Dockerfile -t qhr-profile-showmasterpiece:local .
```
...
`@profile/db` schema 聚合引用各 `packages/*-core` 的 schema；部分 Dockerfile 仍 COPY `app_web/web/src/modules` 以兼容历史构建路径。
