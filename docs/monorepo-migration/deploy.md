...
> 三应用独立镜像 + nginx 同域网关。详见 [MIGRATION-RUNBOOK.md](../../deploy/MIGRATION-RUNBOOK.md)。
...
| `NEXT_PUBLIC_BASE_PATH` | calendar / teach-hub | 网关 `/calendar` 或 `/teach-hub`；独立 dev 留空 |
| `NEXT_PUBLIC_TEACH_HUB_BASE_URL` | teach-hub | 与 `BASE_PATH` 一致（网关 `/teach-hub`） |

## CI

Workflow：`.github/workflows/docker-build-push.yml`

- **matrix** 并行构建并 push 三镜像
...
`@profile/db` schema 仍引用 `apps/web/src/modules/*/db/schema`，故 calendar/teach-hub 构建时 COPY 该目录（无 web 完整 node_modules）。
