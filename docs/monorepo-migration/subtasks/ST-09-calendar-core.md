...
- [x] `app_web/web/src/modules/calendar/` 兼容层（re-export + `CalendarPage` 注入 dateCalculator）
...
2. `calendar-core` 不 import `app_web/web`
3. API 转发 `app_web/web/src/app/api/calendar/*` 不变
