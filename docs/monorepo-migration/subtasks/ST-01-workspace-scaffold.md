...
- [x] 目录占位：`app_web/.gitkeep`、`app_web/README.md`
...
| 路径 | npm name |
|------|----------|
| `app_web/web` | `@profile/web` |
| `app_web/calendar` | `@profile/calendar` |
| `app_web/teach-hub` | `@profile/teach-hub` |
...
- 过早把整个项目挪进 `app_web/web` 会导致路径大面积变更 — **本 ST 禁止搬迁源码**

## 备注

ST-07 才真正搬迁 `src/` → `app_web/web/`；本 ST 仅搭架子。
