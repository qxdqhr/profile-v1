...
flowchart TB
  subgraph apps [apps - 可独立部署]
    WEB[app_web/web]
    CAL[app_web/calendar]
    TH[app_web/teach-hub]
  end
...
| `src/app/.../calendar/**` | `app_web/calendar/app/**` |
| `src/app/api/calendar/**` | `app_web/calendar/app/api/calendar/**` |
| `src/modules/teachHub/**` | `packages/teach-hub-core/**` |
| `src/app/.../teachHub/**` | `app_web/teach-hub/app/**` |
...
- `app_web/*` **只能**依赖 `packages/*`，禁止 `app_web/calendar` → `app_web/web`。
...
- `app_web/calendar` 暴露 `/api/calendar/*`
- `app_web/teach-hub` 暴露 `/api/teach-hub/*`
...
| 耦合点 | 现状 | B 阶段处理 |
|--------|------|------------|
| `registerTeachHubAiTasks` | `aiApi/server/registerCoreTasks.ts` | 迁至 `app_web/teach-hub/instrumentation.ts` 或 `packages/teach-hub-core/server/registerTasks.ts` |
...
| `examples/calendar-demo` | 独立 demo 路由 | 迁入 `app_web/calendar` 或删除，避免 web build 预渲染 |
...
根 `pnpm-workspace.yaml` 纳入 `app_web/*` 与 `packages/*`。迁移前已存在：
...
| B 能力 | C 时怎么用 |
|--------|------------|
| 独立 `app_web/*/Dockerfile` | 直接拆仓，镜像不变 |
