...
**OPT-02 进展（2026-09-14）**：已给 `universal-file`（files/folders 写 + monitoring）、exam 配置写（`examTypes` / `questions`）、`universal-export` 写补 `requireApiSession`。examples 写面多数已有 `requireExampleAccess` / `requireOssExampleAdmin`；补 `qqbot`（NapCat webhook 除外）。余公开 allowlist 面不动。
