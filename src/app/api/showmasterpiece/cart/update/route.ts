/**
 * ShowMasterpiece 模块 - 更新购物车API路由
 * 
 * 处理更新购物车商品数量的HTTP请求
 * 
 * @fileoverview 更新购物车API路由
 */
...
async function PUT(request: NextRequest) {
  try {
    ...
    // 数据验证
    if (!userId || !collectionId || quantity === undefined) {
    ...
    // 更新购物车商品数量
    await CartDbService.updateCartItemQuantity(cart.id, collectionId, quantity);
