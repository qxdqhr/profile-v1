  ...
  return (
    <main className="booth-page min-h-screen p-4 md:p-8">
      ...
      <section className="mx-auto w-full max-w-4xl rounded-3xl border border-pink-200/70 bg-white/85 p-4 shadow-xl backdrop-blur md:p-6">
        <h1 className="text-center text-2xl font-bold text-pink-600 md:text-3xl">🌸 Vocaloid Booth 生产接入页</h1>
        <p className="mt-2 text-center text-sm text-slate-500">
          页面：/vocaloid-booth-test · API：/api/vocaloid-booth
        </p>
        ...
        <div className="mt-4">
          {tab === 'upload' ? (
            <BoothUploadCard onCreated={(next) => setCode(next)} />
          ) : (
            <BoothDownloadCard initialCode={code} />
          )}
        </div>
      </section>
