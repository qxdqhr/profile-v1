  ...
  const handleUniversalUpload = async (file: File) => {
    ...
    try {
      const result = await uploadModuleFile({
        ...
        folderPath: businessType === 'cover'
          ? `showmasterpiece/cover/${Date.now()}_${Math.random().toString(36).slice(2, 11)}.${file.name.split('.').pop()?.toLowerCase() || 'jpg'}`
          : undefined,
        needsProcessing: true,
      });

      onChange({
        image: result.accessUrl || `/api/universal-file/${result.fileId}`,
