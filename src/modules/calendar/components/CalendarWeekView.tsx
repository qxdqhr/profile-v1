          ...
          return (
            <div
              ...
              className={`py-2 text-center text-xs font-semibold sm:text-sm ${
                isWeekend ? 'text-red-600' : 'text-slate-700'
              }`}
            >
              <span className="hidden sm:inline">{getWeekdayName(date, 'zh-CN', 'short')}</span>
              <span className="sm:hidden">{getWeekdayName(date, 'zh-CN', 'short').slice(-1)}</span>
            </div>
          );
        })}
      </div>
        ...
        {weekDates.map((date) => {
          ...
          return (
            <button
              ...
              className={`flex min-h-[7rem] flex-col p-1.5 text-left transition-colors hover:bg-violet-50/50 sm:min-h-[9rem] sm:p-2 ${
                isTodayDate ? 'bg-violet-50/80' : 'bg-white'
              }`}
            >
              ...
              <div className="flex flex-1 flex-col gap-0.5 overflow-hidden">
                ...
                {overflow > 0 && (
                  <span
                    ...
                    className="rounded-md px-1 py-0.5 text-center text-[10px] font-medium text-violet-700 hover:bg-violet-100 sm:text-xs"
                  >
                    +{overflow} 项
                  </span>
                )}
