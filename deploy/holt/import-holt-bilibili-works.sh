#!/usr/bin/env bash
# 将 app_wordpress/holt/data/holt-bilibili-works.json 导入 wordpress_holt（仅数据，不改主题）。
# 用法（在 /root/profile-v1）：
#   ./holt/import-holt-bilibili-works.sh
# 可选：
...
if [ -z "$JSON_HOST" ]; then
  for cand in \
    "$DEPLOY_DIR/wordpress/holt/data/holt-bilibili-works.json" \
    "$DEPLOY_DIR/../app_wordpress/holt/data/holt-bilibili-works.json"; do
    if [ -f "$cand" ]; then
      JSON_HOST="$cand"
      break
    fi
...
if [ -z "${JSON_HOST:-}" ] || [ ! -f "$JSON_HOST" ]; then
  echo "ERROR: missing holt-bilibili-works.json (set JSON_HOST=...)" >&2
  exit 1
fi
