...
if [ -x "$GATEWAY_DIR/ensure-database-url.sh" ]; then
  FIXED_URL="$("$GATEWAY_DIR/ensure-database-url.sh")"
elif [ -f "$GATEWAY_DIR/ensure-database-url.sh" ]; then
  FIXED_URL="$(bash "$GATEWAY_DIR/ensure-database-url.sh")"
else
  FIXED_URL=""
fi
...
if [ -n "$FIXED_URL" ]; then
  ...
  if [ "${POST_DEPLOY:-}" = "1" ] && [ "$OLD_URL" != "$FIXED_URL" ]; then
    ...
    if [ -x "$GATEWAY_DIR/wait-gateway-ready.sh" ]; then
      ...
      GATEWAY_PORT="${GATEWAY_PORT:-3000}" bash "$GATEWAY_DIR/wait-gateway-ready.sh"
    else
      sleep 20
    fi
...
if [ -x "$GATEWAY_DIR/ensure-calendar-schema.sh" ]; then
  "$GATEWAY_DIR/ensure-calendar-schema.sh"
elif [ -f "$GATEWAY_DIR/ensure-calendar-schema.sh" ]; then
  bash "$GATEWAY_DIR/ensure-calendar-schema.sh"
fi

echo
echo "========== 5d. node-notes 表结构 =========="
if [ -x "$GATEWAY_DIR/ensure-node-notes-schema.sh" ]; then
  "$GATEWAY_DIR/ensure-node-notes-schema.sh"
elif [ -f "$GATEWAY_DIR/ensure-node-notes-schema.sh" ]; then
  bash "$GATEWAY_DIR/ensure-node-notes-schema.sh"
fi
...
echo
echo "========== 8. 外层 nginx 审计 =========="
if [ -x "$GATEWAY_DIR/audit-outer-nginx.sh" ]; then
  "$GATEWAY_DIR/audit-outer-nginx.sh"
elif [ -f "$GATEWAY_DIR/audit-outer-nginx.sh" ]; then
  bash "$GATEWAY_DIR/audit-outer-nginx.sh"
fi
...
elif [ -x "$GATEWAY_DIR/smoke-test-gateway.sh" ]; then
