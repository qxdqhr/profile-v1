#!/usr/bin/env bash
# ShowMasterpiece 整体打包：Web Docker 镜像
# 与 CI docker-build-push 对齐。
set -euo pipefail
...
usage() {
  cat <<'EOF'
用法: bash deploy/scripts/docker/showmasterpiece-docker-package.sh [IMAGE_TAG]
...
if [ "${BUILD_DOCKER}" = "1" ]; then
  ...
  docker buildx build --platform "${PLATFORM}" \
    -f app_web/showmasterpiece/Dockerfile \
...
echo ""
echo "==> ShowMasterpiece package summary"
echo "Docker image : qhr-profile-showmasterpiece:${IMAGE_TAG}"

if [ "${SEND_FEISHU}" = "1" ] || [ -n "${FEISHU_WEBHOOK_URL:-}" ] || [ -n "${CI_FEISHU_WEBHOOK_URL:-}" ]; then
  echo "==> Send Feishu notification"
  CI_NOTIFY_STATUS=success \
  CI_IMAGE_TAG="showmasterpiece:${IMAGE_TAG}" \
  GITHUB_WORKFLOW="${GITHUB_WORKFLOW:-ShowMasterpiece Package}" \
  GITHUB_REPOSITORY="${GITHUB_REPOSITORY:-local/showmasterpiece}" \
