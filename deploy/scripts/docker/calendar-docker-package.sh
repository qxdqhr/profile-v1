...
usage() {
  cat <<'EOF'
用法: bash deploy/scripts/docker/calendar-docker-package.sh [IMAGE_TAG]
...
if [ "${BUILD_ANDROID}" = "1" ]; then
  ...
  bash app_mobile/calendar-mobile/scripts/android-release-build.sh
  APK_FILE="$(ls -t "${ROOT_DIR}"/app_mobile/calendar-mobile/dist/*.apk 2>/dev/null | head -1 || true)"
  if [ -z "${APK_FILE}" ] || [ ! -f "${APK_FILE}" ]; then
    echo "ERROR: Android APK not found under app_mobile/calendar-mobile/dist/" >&2
    exit 1
  fi
