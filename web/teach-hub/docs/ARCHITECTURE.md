...
> 视角：`web/teach-hub`（Next.js 子应用）+ `packages/teach-hub-core`（业务包）  
...
| 层 | 包 | 职责 |
|----|-----|------|
| 子应用壳 | `web/teach-hub` | 路由、layout、API re-export、Docker、basePath |
| 业务核心 | `packages/teach-hub-core` | 页面、组件、服务、API handler、AI 任务 |
| 主站 | `web/web` | `/teach-hub` 重定向、实验田卡片，**不**直接依赖 core |
...
Docker 构建：`docker build -f web/teach-hub/Dockerfile .`
...
| 子应用路由 | `web/teach-hub/app/` |
| AI 任务注册 | `web/teach-hub/instrumentation.ts` |
| 样式常量 | `styles/tw.ts` |
