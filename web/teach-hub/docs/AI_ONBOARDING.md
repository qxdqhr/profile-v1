...
```
profile-v1/
  web/teach-hub/              ← 子应用（路由壳、部署）  ★ 你在这里
  packages/teach-hub-core/     ← 业务逻辑 + UI + API 实现  ★ 改功能主要在这里
  web/web/                    ← 主站，/teach-hub 只做 302 重定向
```
...
| P2 | `web/teach-hub/instrumentation.ts` | AI 任务注册入口 |
| P2 | `web/teach-hub/next.config.ts` | 构建/环境 |
...
2. **AI 任务注册**：`teach.generateLesson` 只在 `web/teach-hub/instrumentation.ts` 注册；改主站 `registerCoreTasks` 无效。
