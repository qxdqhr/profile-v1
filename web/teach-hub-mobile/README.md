...
与 `web/teach-hub`（Web 子应用）同级，共享 `npm/teach-hub-shared`。
...
2. 配置环境变量：
   ...
   ```bash
   cp web/teach-hub-mobile/.env.example web/teach-hub-mobile/.env
   ```
...
桌面端脚手架：`web/teach-hub-desktop`（Electron + Vite，复用 `teach-hub-shared`）
...
# 产物：web/teach-hub-mobile/dist/teach-hub-mobile-<version>+<build>.apk
```
