...
Godot 源码在 `games/<slug>/`，旁路见 `deploy/games/`（现网：`pulse-parade`、`flappy-wish`；CI `export-godot-games`）。小游戏迁移为**双轨**（原版保留 + Godot 最简并行），见 `deploy/games/GODOT-REWRITE-PLAN.md`。
