...
static func one_corner(start: Vector2i, end: Vector2i, grid: Array) -> Array:
	...
	if not bool((grid[c1.y] as Array)[c1.x]) and can_connect_directly(start, c1, grid) and can_connect_directly(c1, end, grid):
		return [start, c1, end]
	var c2 := Vector2i(start.x, end.y)
	if not bool((grid[c2.y] as Array)[c2.x]) and can_connect_directly(start, c2, grid) and can_connect_directly(c2, end, grid):
		return [start, c2, end]
	return []
...
static func two_corner(start: Vector2i, end: Vector2i, grid: Array) -> Array:
	...
	for x in w:
		...
		if not bool((grid[c1.y] as Array)[c1.x]) and not bool((grid[c2.y] as Array)[c2.x]):
			if can_connect_directly(start, c1, grid) and can_connect_directly(c1, c2, grid) and can_connect_directly(c2, end, grid):
...
## board[row][col] = type (>=0) or -1 empty. Returns path in board coords (with pad offset).
static func try_link(board: Array, a: Vector2i, b: Vector2i, pad: int = 2) -> Dictionary:
	if a == b:
		return {"ok": false}
