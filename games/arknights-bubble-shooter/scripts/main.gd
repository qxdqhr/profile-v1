extends Node2D
## 岁家龙泡泡 — dual-track Godot port (same logic as bubble-shooter, Arknights palette).
...
func _draw() -> void:
	...
	draw_line(Vector2(4, danger_y), Vector2(bw - 4, danger_y), Color(0.776, 0.157, 0.157, 0.45), 2.0)
	...
	if _status == Status.READY:
		_draw_bubble(Vector2(shooter.x, shooter.y - BubbleConfig.RADIUS), _current_color)
		_draw_bubble(Vector2(shooter.x + 36, shooter.y - 8), _next_color, BubbleConfig.RADIUS * 0.55)
		var px := shooter.x
		...
		for i in 6:
			...
			if py < BubbleConfig.TOP_OFFSET + BubbleConfig.RADIUS:
				break
			draw_circle(Vector2(px, py), 2.5, Color(GOLD.r, GOLD.g, GOLD.b, 0.45))

	if not _projectile.is_empty():
		_draw_bubble(Vector2(float(_projectile["x"]), float(_projectile["y"])), int(_projectile["color"]))

func _draw_bubble(center: Vector2, idx: int, radius: float = BubbleConfig.RADIUS) -> void:
	var color := BubbleConfig.color_for(idx)
	var light := BubbleConfig.highlight_for(idx)
	draw_circle(center, radius, color)
	draw_circle(center + Vector2(-radius * 0.28, -radius * 0.32), radius * 0.32, Color(light.r, light.g, light.b, 0.35))
	draw_arc(center, radius, 0, TAU, 24, Color(light.r, light.g, light.b, 0.55), 1.2)
