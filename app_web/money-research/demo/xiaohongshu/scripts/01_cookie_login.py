...
def main() -> int:
    parser = argparse.ArgumentParser(description="小红书 Cookie 登录检测")
    ...
    if args.save and result.get("logged_in"):
        save_cookies(
            Path(args.cookie_file),
            client.cookies,
            extra={"nickname": result.get("nickname"), "user_id": result.get("user_id")},
        )
