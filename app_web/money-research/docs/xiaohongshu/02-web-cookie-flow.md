...
```
GET  /api/sns/app_web/v2/user/me          # 当前登录用户
GET  /api/sns/app_web/v1/user/selfinfo    # 账号信息（旧）
```

### 内容读取

```
POST /api/sns/app_web/v1/search/notes     # 搜索笔记
GET  /api/sns/app_web/v1/feed             # 推荐流
GET  /api/sns/app_web/v1/note/{id}        # 笔记详情
GET  /api/sns/app_web/v2/comment/page     # 评论列表
GET  /api/sns/app_web/v1/user/{id}        # 用户主页
```

### 互动写入

```
POST /api/sns/app_web/v1/note/like        # 点赞
POST /api/sns/app_web/v1/note/collect     # 收藏
POST /api/sns/app_web/v1/user/follow      # 关注
POST /api/sns/app_web/v1/comment/post     # 评论
```

### 创作者发布（重点）

```
GET  /api/media/v1/upload/app_web/permit?biz_name=spectrum&scene=image&file_count=1
