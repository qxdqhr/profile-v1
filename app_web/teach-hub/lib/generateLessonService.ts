...
import type { GenerateTrigger, TeachGenerateJob, GenerateLessonOutput } from 'sa2kit/business/teachHub/domain';
import { parseMissionMarkdown } from 'sa2kit/business/teachHub/domain';
import { getNextLessonOrder } from './lessonIndex';
import {
...
export async function runGenerateLesson(input: {
  ...
  try {
    ...
    const { output } = result.data as { output: GenerateLessonOutput; rawText: string };
    const paths: string[] = [];
