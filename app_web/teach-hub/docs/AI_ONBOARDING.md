...
```
profile-v1/
  app_web/teach-hub/              ← 子应用（路由壳、部署）  ★ 你在这里
  packages/teach-hub-core/     ← 业务逻辑 + UI + API 实现  ★ 改功能主要在这里
  app_web/web/                    ← 主站，/teach-hub 只做 302 重定向
```
...
要改路由/部署/basePath？
  └─ app_web/teach-hub/app/
  └─ app_web/teach-hub/next.config.ts
...
| P2 | `app_web/teach-hub/instrumentation.ts` | AI 任务注册入口 |
| P2 | `app_web/teach-hub/next.config.ts` | 构建/环境 |
...
2. **AI 任务注册**：`teach.generateLesson` 只在 `app_web/teach-hub/instrumentation.ts` 注册；改主站 `registerCoreTasks` 无效。
